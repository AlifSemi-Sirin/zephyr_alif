 ...
 *                                                                                                          *
 * Memory information (allocations etc.) is stored inside a control block. * The control block
 ...
 *                                                                                                        *
 * Parameters: * ctrlblk :   pointer to the heap contol block. * heapaddr :  base address of the
...
void d0_fixed_range_fixed_blkcnt_heapinit( void* ctrlblk, void* heapaddr, unsigned int heapsize, unsigned int maxblocks)
{
	...
	heap->usedblocks = (d0_fixed_range_fixed_blk_memblock
				    *)((char *)heap->freeblocks +
				       maxblocks * sizeof(d0_fixed_range_fixed_blk_memblock));
 ...
 *                                                                                                          *
 * Parameters: * ctrlblk :  pointer to the heap contol block. * size:      requested size in bytes *
 *                                                                                                        *
 * Return Value: * NULL in case of an error, a pointer to the reserved memory otherwise. *
 */
 ...
 *                                                                                                          *
 * Parameters: * ctrlblk :  pointer to the heap contol block. * ptr:       pointer to the memory
 * block                                                                  *
 *                                                                                                        *
...
d0_fixed_range_fixed_blkcnt_heapfree(void *ctrlblk, void *ptr)
{
	...
	if (used_blk->addr == ptr) {
		...
		d0_fixed_range_fixed_blk_memblock *empty_free =
			NULL; /* unused entry in free list */

		/* find empty block/possibility to merge in free list */
		while ((!(pred_free && succ_free)) && (free_blk <= last_free)) {
			if (free_blk->addr) {
				if ((!pred_free) &&
				    (((char *)(free_blk->addr) + free_blk->size) == ptr)) {
					pred_free = free_blk;
				}

				if ((!succ_free) &&
				    ((((char *)ptr) + used_blk->size) == free_blk->addr)) {
					succ_free = free_blk;
				}
			} else if (!empty_free) {
		...
		/* now performe merge (in front of successor if possible, because that is the
		   direction, from where we allocate) */
		if (succ_free) {
			succ_free->addr = ptr;

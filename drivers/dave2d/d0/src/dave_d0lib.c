...
#if !defined(WITH_MM_FIXED_RANGE_FIXED_BLKCNT) && !defined(WITH_MM_FIXED_RANGE) &&                 \
 ...
 *****************************************************************************/
void *d0_dummy_alloc(void *ctrlblk, unsigned int size)
{
	(void)ctrlblk;
	(void)size;
	return NULL;
}
void d0_dummy_free(void *ctrlblk, void *ptr)
{
	(void)ctrlblk;
	(void)ptr;
}
unsigned int d0_dummy_memsize(void *ctrlblk, void *ptr)
{
	(void)ctrlblk;
	(void)ptr;
	return 0;
 ...
 *****************************************************************************/
int d0_initheapmanager(void *heapaddress, unsigned int heapsize, d0_memmanager_type heapmgrtype,
		       void *vidmemaddress, unsigned int vidmemsize, unsigned int vidmemblkcnt,
		       d0_memmanager_type vidmemmgrtype, d0_memarchitectures flags)
{
	/* check arguments */
	if (!heapsize) {
		return 0;
	...
	if (flags & d0_ma_separated) {
		if (vidmemsize) {
			...
			if (vidmemmgrtype == d0_mm_fixed_range_fixed_blkcnt) {
				videomemory->ctrlblk = d0_allocmem(
					get_fixed_range_fixed_blkcnt_ctrlblksize(vidmemblkcnt));
			...
			d0_preparememorymanager(videomemory, vidmemmgrtype, vidmemaddress,
						vidmemsize, videomemory->ctrlblk, vidmemblkcnt);
		} else {
			return 0;
		}
	} else {
...
void *d0_allocvidmem(unsigned int size)
{
	...
	if (!videomemory->allocmem || !videomemory->ctrlblk) {
		return NULL;
	}

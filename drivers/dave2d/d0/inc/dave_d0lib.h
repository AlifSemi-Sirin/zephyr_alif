 ...
 *                                     This memory manager needs 28 bytes of fixed control
 *information and 16 bytes for each block of allocatable memory.
 *
 * Topic: Usage
...
typedef enum {
	...
	d0_ma_separated = 0x1 /* Video memory is separated from system memory and can be accessed by
				 transfer or mapping functions only. */
} d0_memarchitectures;
 ...
 *****************************************************************************/
extern void d0_preparememorymanager(d0_memmanager *mgr, d0_memmanager_type memmanagertype,
				    void *base, unsigned int size, void *ctrlblk,
				    unsigned int blkcnt);

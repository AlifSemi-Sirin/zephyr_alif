					    ...
					    d2_u32 usehiprecision)
{
	...
	if (0 == usehiprecision) {
		...
		D2_DLISTWRITES(D2_L1START + index, -s << LIMITER_HIPRECISION);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		D2_DLISTWRITES(D2_L1XADD + index, grad->xadd << LIMITER_HIPRECISION);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		D2_DLISTWRITES(D2_L1YADD + index, grad->yadd << LIMITER_HIPRECISION);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
	}
			       ...
			       const d2_bbox *bbox, d2_u32 limindex, d2_u32 control)
{
	if (0 != (ctx->gradients & 1)) {
		if (0 != (ctx->gradient[0].mode & d2_grad_linear)) {
			...
			control |= d2_initgradient_lin(handle, &ctx->gradient[0], bbox, limindex,
						       control & D2C_LIMITERPRECISION);
	...
	if (0 != (ctx->gradients & 2)) {
		if (0 != (ctx->gradient[1].mode & d2_grad_linear)) {
			...
			control |= d2_initgradient_lin(handle, &ctx->gradient[1], bbox, limindex,
						       control & D2C_LIMITERPRECISION);
			control &= ~D2C_SPANSTORE;
	...
	if ((0 != (ctx->alphamode & d2_am_gradient1)) && (0 != (ctx->gradients & 4))) {
		if (0 != (ctx->gradient[2].mode & d2_grad_linear)) {
			...
			control |= d2_initgradient_lin(handle, &ctx->gradient[2], bbox, limindex,
						       control & D2C_LIMITERPRECISION);
			control &= ~D2C_SPANSTORE;
	...
	if ((0 != (ctx->alphamode & d2_am_gradient2)) && (0 != (ctx->gradients & 8))) {
		if (0 != (ctx->gradient[3].mode & d2_grad_linear)) {
			...
			control |= d2_initgradient_lin(handle, &ctx->gradient[3], bbox, limindex,
						       control & D2C_LIMITERPRECISION);
			control &= ~D2C_SPANSTORE;

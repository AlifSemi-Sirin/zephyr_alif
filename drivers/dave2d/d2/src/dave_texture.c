 ...
 *   d2_mode_ai44     - colored, palletized 8bit per pixel, (4 bit alpha, 4 bit indexed RGB: see
 * <d2_settexclut>) d2_mode_i8        - colored, palletized 8bit per pixel (palette is used if
 * d2_mode_clut is also set) d2_mode_i4        - colored, palletized 4bit per pixel (palette is used
 * if d2_mode_clut is also set) d2_mode_i2        - colored, palletized 2bit per pixel (palette is
 * used if d2_mode_clut is also set) d2_mode_i1        - colored, palletized 1bit per pixel (palette
 * is used if d2_mode_clut is also set)
 *
 * additional flags (can be combined with all above formats) :
 *
 *   d2_mode_clut     -  Enables the color look up table (for d2_mode_i8 through d2_mode_i1) (see
 * <d2_settexclut>).
 *
 * Modes d2_mode_rgb888, d2_mode_rgb444 and d2_mode_rgb555 can be used as well.
 * In this case alpha information has to be ignored by setting a <d2_settextureoperation> of
 * d2_to_one for alpha.
 *
 * If the CLUT is not enabled with an indexed color format (d2_mode_i8 to d2_mode_i1), the index is
 * written directly to the framebuffer. The index can be combined with the offset, see
 * <d2_settexclut_offset>.
 *
 * In case of d2_mode_alpha4 to d2_mode_alpha1 bits are MSB aligned and replicated to the lower bits
 * of the 8 bit format of the internal A, R, G and B channels.
 *
 * returns:
		     ...
		     d2_u32 format)
{
	...
	D2_CHECKERR(width >= 0, D2_VALUENEGATIVE);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(height >= 0, D2_VALUENEGATIVE);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	if (((d2_mode_alpha4 == format_noflags) || (d2_mode_alpha2 == format_noflags) ||
	...
	case d2_mode_alpha1:
		ctx->tbstylemask =
			D2C_READFORMAT4 | D2C_READFORMAT3 | D2C_READFORMAT2 | D2C_READFORMAT1;
		ctx->texbpp = 1;
		ctx->rlebpp = 1;
		ctx->texsubppb = 8;
		break;
...
d2_s32 d2_settextureoperation(d2_device *handle, d2_u8 amode, d2_u8 rmode, d2_u8 gmode, d2_u8 bmode)
{
	...
	if (NULL != handle) {
		...
		D2_CHECKERR((amode < 8u), D2_INVALIDENUM);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		D2_CHECKERR((rmode < 8u), D2_INVALIDENUM);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		D2_CHECKERR((gmode < 8u), D2_INVALIDENUM);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		D2_CHECKERR((bmode < 8u), D2_INVALIDENUM);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
...
d2_s32 d2_settexopparam(d2_device *handle, d2_u32 index, d2_u32 p1, d2_u32 p2)
{
	...
	if (NULL != handle) {
		...
		D2_CHECKERR(p1 < 256, D2_VALUETOOBIG);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		D2_CHECKERR(p2 < 256, D2_VALUETOOBIG);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
 ...
 * It is read once before the next object using textures is rendered or if <d2_settexclut_part> is
 * used. If the CLUT is changed later on, d2_settexclut has to be called again to trigger a new
 * upload.
 *
 *
 * parameters:
 ...
 *
 *   the parameter clut can be NULL. If the CLUT was cached then the allocated memory will be freed
 * (see: <d2_settexclut_part>)
 *
 * see also:
	...
	if (NULL == clut) {
		...
		if (NULL != D2_DEV(handle)->ctxselected->texclut_cached) {
			for (i = 0; i < clut_entries; i++) {
				D2_DEV(handle)->ctxselected->texclut_cached[i] =
					D2_DEV(handle)->ctxselected->texclut[i];
			}
		}
 ...
 * If a pointer to a CLUT was given by <d2_settexclut> this CLUT contents is first copied to the
 * context. The contents of clut_part is then copied to the cached CLUT starting at start_index with
 * a length of length.
 *
 * The size of the CLUT is 256 resp. 16 (if feature bit D2FB_TEXCLUT256 = 1 resp. = 0 (see
 * <d2_getrevisionhw>))
 *
 * parameters:
 *   handle      - device pointer (see: <d2_opendevice>)
			  ...
			  d2_u32 length)
{
	...
	D2_CHECKERR(start_index < max_index, D2_VALUETOOBIG);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(length > 0, D2_VALUENEGATIVE);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(length <= max_index, D2_VALUETOOBIG);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR((start_index + length) <= max_index, D2_VALUETOOBIG);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
 ...
 * The contents of clut_part is written directly to the render buffer starting at start_index with a
 * length of length.
 *
 * The size of the CLUT is 256 resp. 16 (if feature bit D2FB_TEXCLUT256 = 1 resp. = 0 (see
 * <d2_getrevisionhw>))
 ...
 *
 * The offset is an 8 bit resp. 4 bit value (if feature bit D2FB_TEXCLUT256 = 1 resp. = 0 (see
 * <d2_getrevisionhw>))
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
...
d2_s32 d2_settexclut_offset(d2_device *handle, d2_u32 offset)
{
	...
	if (0 == (D2_DEV(handle)->hwrevision & D2FB_TEXCLUT256)) {
		D2_CHECKERR(offset < 16, D2_VALUETOOBIG);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		offset &= 0x0fu;
	}
 ...
 *
 * The Color lookup table has 256 entries of 32 bit color values. Each color value is coded as
 * ARGB8888. In case of RGB565 the CLUT 256x32bit is divided into a lower and an upper part for 256
 * 16bit color entries. One 32bit word contains two successive 16bit color entries. The lower 16 bit
 * contain the first color entry and the upper 16 bit contain the next color entry. The upper part
 * of the CLUT can be accessed by the texture unit by setting the texclut_offset to 0x80. When
 * loading the CLUT 32bit words are written to the RAM. In this 16bit mode the driver has to take
 * care to write the correct data.
 *
 * If the feature bit D2FB_TEXCLUT256 = 0 the format applies to 16 entries of 32 bit (see
 * <d2_getrevisionhw>).
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
 ...
 * If the values are equal then Alpha as well as R, G and B are set to 0 to mark the pixel
 * transparent.
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
					 ...
					 d2_alpha *c1, d2_alpha *c2)
{
	...
	case d2_to_replace:
		*c1 = ctx->texop_p1[index];
		*c2 = ctx->texop_p1[index];
		break;
					       ...
					       const d2_bbox *bbox)
{
	...
	s1 = -(((x * ctx->texlim[0].xadd) + (y * ctx->texlim[0].yadd)) >> 4);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	s2 = -(((x * ctx->texlim[1].xadd) + (y * ctx->texlim[1].yadd)) >> 4);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	...
	D2_DLISTWRITES(D2_LVSTARTI, (s2 >> 16) * ctx->texpitch);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	D2_DLISTWRITEU(D2_LVSTARTF, (d2_u16)s2);

	D2_DLISTWRITES(D2_LVXADDI, (xa >> 16) * ctx->texpitch);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	D2_DLISTWRITES(D2_LVYADDI, (ya >> 16) * ctx->texpitch);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/

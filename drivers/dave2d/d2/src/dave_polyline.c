...
static d2_s32 d2_renderpolyline2_intern(d2_devicedata *handle, d2_contextdata *ctx,
					...
					d2_point soffy, d2_width olwidth); /* MISRA */
			...
			if (0 != joins) {
				...
				if (d2_le_exclude_end == (sflag & d2_le_exclude_end)) {
					...
					if (i ==
					    (count -
					     1)) /* PRQA S 3382 */ /* $Misra:
								      #MISRA_BUG_ZERO_WRAPAROUND $*/
					{
						/* wrap back to start */
						d2_fixlength(px2, py2,
							     (d2_point)(pstart[0] + soffx),
							     (d2_point)(pstart[1] + soffy), &dxp1,
							     &dyp1);
					} else {
						/* find future delta */
						d2_fixlength(px2, py2, (d2_point)(data[0] + soffx),
							     (d2_point)(data[1] + soffx), &dxp1,
							     &dyp1);
					}
				...
				if (0 == bSkipSeg) {
					...
					if (0 != dots) {
						...
						if ((i > 0) || (0 != (flags & d2_le_closed))) {
							(void)d2_renderlinedot_intern(
								handle, ctx, px1, py1, w, 1,
...
static d2_s32 d2_renderpolyline2_intern(d2_devicedata *handle, d2_contextdata *ctx,
					const d2_point *data, const d2_u32 *sflags, d2_u32 count,
					const d2_width *w, d2_u32 flags, d2_point soffx,
					d2_point soffy, d2_width olwidth)
{
	d2_u32 i;
	d2_point px1, py1;
	d2_point px2, py2;
	...
	for (i = 0; i < count; i++) {
		...
		if (0 == bSkip) {
			...
			if (0 == bSkipSeg) {
				...
				/* render segment */
				d2_u32 temp_flag = d2_le_exclude_start |
						   d2_le_exclude_end; /* default is not to draw
									 start and end caps */

				if ((0 == i) && (0 == (flags & d2_le_exclude_start)) &&
				    (0 == (flags & d2_le_closed))) {
					temp_flag &= ~d2_le_exclude_start; /* draw start cap */
				}

				if ((i == (count - 1)) && (0 == (flags & d2_le_exclude_end)) &&
				    (0 == (flags & d2_le_closed))) {
					temp_flag &= ~d2_le_exclude_end; /* draw end cap */
				}

				(void)d2_renderline2_intern(handle, ctx, px1, py1, px2, py2, w1, w2,
							    temp_flag, edge_buffer, &edge_bbox);

				if ((i > 0) || (0 != (flags & d2_le_closed))) {
					(void)d2_renderlinedot_intern(
						handle, ctx, px1, py1, w1, 0, edge_buffer,
						&edge_bbox); /* render a dot at line startpos */
				}
			}
...
d2_s32 d2_renderpolyline_solid(d2_device *handle, const d2_point *data, d2_u32 count, d2_width w,
			       d2_u32 flags)
{
	(void)d2_renderpolyline_intern(D2_DEV(handle), D2_DEV(handle)->ctxsolid, data, NULL, count,
				       w, flags, 0, 0);
				...
				d2_u32 flags)
{
	...
	(void)d2_renderpolyline_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, data, NULL,
				       count, w, flags, sx, sy);
...
{
	(void)d2_renderpolyline_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, data, NULL,
				       count, (d2_width)(w + (D2_DEV(handle)->outlinewidth * 2)),
				       flags, 0, 0);
...
{
	(void)d2_renderpolyline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxsolid, data, NULL, count,
					w, flags, 0, 0, 0);
				 ...
				 const d2_width *w, d2_u32 flags)
{
	...
	(void)d2_renderpolyline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, data, NULL,
					count, w, flags, sx, sy, 0);
				  ...
				  const d2_width *w, d2_u32 flags)
{
	(void)d2_renderpolyline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, data, NULL,
					count, w, flags, 0, 0,
					(d2_width)(D2_DEV(handle)->outlinewidth * 2));

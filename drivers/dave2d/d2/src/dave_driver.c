 ...
 *  d2_df_no_blitctxbackup   - don't backup context data at blit for better performance; previous
 * texture modes get lost and must be set again
 *
 * note:
 *  Flags 'd2_df_no_fbcache' and 'd2_df_no_texcache' should only be used for debugging purposes.
 *  Especially when multithreading is used, these flags must not be set different for devices which
 * are bound to the same hardware instance through <d2_inithw>.
 *
 * returns:
...
#else
		handle->blitcontext_b = d2_newbackupcontext_intern(
			handle, flags); /* create backup for blit context */
#endif
...
d2_s32 d2_inithw(d2_device *handle, d2_u32 flags)
{
	...
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(d2_dev->hwid == NULL, D2_DEVASSIGNED);
	...
	if (0 != d2_dev->lowlocalmem_mode) {
		...
		D2_CHECKERR(llm_mode->dlist_buffer, D2_NOMEMORY);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
		llm_mode->dlist_buffer->next =
			d2_alloc_dlistblock_intern(handle, d2_dev->dlistblocksize);
		D2_CHECKERR(llm_mode->dlist_buffer->next, D2_NOMEMORY);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	d2_dev->dlscratch_base = (d2_dlist_scratch_entry *)d2_getmem_p(
		D2_DLISTSCRATCH * sizeof(d2_dlist_scratch_entry));
	D2_CHECKERR(d2_dev->dlscratch_base, D2_NOMEMORY);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
...
d2_s32 d2_deinithw(d2_device *handle)
{
	...
	if (NULL != D2_DEV(handle)->hwid) {
		...
		if (0 != D2_DEV(handle)->lowlocalmem_mode) {
			...
			d2_free_dlistblock_intern(handle,
						  D2_DEV(handle)->lowlocalmem_mode->dlist_buffer);
		}

		/* free display lists */
		rb1 = D2_DEV(handle)->renderbuffer[0];
		rb2 = D2_DEV(handle)->renderbuffer[1];
		D2_DEV(handle)->renderbuffer[0] =
			NULL; /* necessary in oder to free default buffer */
...
{
	d2hw_set(D2_DEV(handle)->hwid, D2_CONTROL3,
		 burstlengthFBread | (burstlengthFBwrite << 8) | (burstlengthTX << 16) |
			 (burstlengthDL << 24));
	D2_RETOK(handle);
}
...
const d2_char *d2_getrevisionstringhw(const d2_device *handle)
{
	...
	static const d2_char g_hwrevision[3][16] = {
		{"D/AVE 2DT-S"}, {"D/AVE 2DT-L"}, {"D/AVE UNKNOWN"}};
 ...
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
 *   dlistblockfactor - size of a dlist block in vidmem is this factor * local size (configurable
 * using <d2_setdlistblocksize>) dlistblocks - maximum number of dlist blocks in vidmem
 *
 * returns:
...
d2_s32 d2_lowlocalmemmode(d2_device *handle, d2_u32 dlistblockfactor, d2_u32 dlistblocks)
{
	...
	D2_CHECKERR(dlistblockfactor >= 2, D2_VALUETOOSMALL);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(dlistblocks >= 1, D2_VALUETOOSMALL);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	if (NULL == D2_DEV(handle)->lowlocalmem_mode) {
		...
		D2_CHECKERR(D2_DEV(handle)->lowlocalmem_mode, D2_NOMEMORY);
			/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	}
...
d2_s32 d2_selectrendermode(d2_device *handle, d2_u32 mode)
{
	...
	/*D2_VALIDATE( handle, D2_INVALIDDEVICE );*/ /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO
										$*/
	D2_CHECKERR(((mode & (d2_rm_solid | d2_rm_outline | d2_rm_shadow | d2_rm_postprocess)) ==
		     mode) &&
			    (mode != 0),
		    D2_ILLEGALMODE);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
...
{
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(width >= 0, D2_INVALIDWIDTH);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
...
d2_u32 d2_getdlistblockcount(d2_device *handle)
{
	...
	if (NULL != dlist->vidmem_blocks) {
		num += ((d2_u32)(dlist->vidmem_blocks->currentblock -
				 dlist->vidmem_blocks->blocks)) *
		       dlist->vidmem_blocks->num_slices;

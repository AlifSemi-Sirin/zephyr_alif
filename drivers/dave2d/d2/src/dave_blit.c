...
#define D2_CHECKERR_BLITHEIGHT(y)                                                                  \
	D2_CHECKERR((y) <= 1024, D2_INVALIDHEIGHT)                                                 \
	/* PRQA S 4130, 3112, 3453 */ /* $Misra: #DEBUG_MACRO $*/
#define D2_CHECKERR_BLITPITCH(p)                                                                   \
	D2_CHECKERR((p) < 65536, D2_VALUETOOBIG)                                                   \
	/* PRQA S 4130, 3112, 3453 */ /* $Misra: #DEBUG_MACRO $*/
#define D2_CHECKERR_BLITSRCPOSX(x)                                                                 \
	D2_CHECKERR((p) < 65536, D2_VALUETOOBIG)                                                   \
	/* PRQA S 4130, 3112, 3453 */ /* $Misra: #DEBUG_MACRO $*/
 ...
 *   d2_mode_rgb888   -  (same as d2_mode_argb8888, in case of RLE textures 24bit pixel are decoded
 * to 32bit) d2_mode_argb4444 - colored 16bit per pixel d2_mode_rgba4444 - colored 16bit per pixel
 ...
 *
 *   d2_mode_rle      -  Enables the RLE unit (available if the feature bit D2FB_RLEUNIT is set (see
 * <d2_getrevisionhw>)). d2_mode_clut     -  Enables the color look up table (for d2_mode_i8 through
 * d2_mode_i1) (see <d2_settexclut>).
 *
 * Modes d2_mode_rgb888, d2_mode_rgb444 and d2_mode_rgb555 can be used as well. Alpha information
 * has to be ignored by not selecting the flag d2_bf_usealpha when calling <d2_blitcopy>.
 *
		     ...
		     d2_u32 format)
{
	...
	if ((0 != (format & d2_mode_clut)) &&
	    (0 == ((D2_DEV(handle)->hwrevision) & D2FB_TEXCLUT))) {
		D2_RETERR(handle, D2_ILLEGALMODE);
	}

	if (((format_noflags == d2_mode_alpha4) || (format_noflags == d2_mode_alpha2) ||
	     (format_noflags == d2_mode_alpha1)) &&
	    ((0 == (D2_DEV(handle)->hwrevision & D2FB_SWDAVE)) &&
	     ((D2_DEV(handle)->hwrevision & 0xff) < 0x0a))) {
		D2_RETERR(handle, D2_ILLEGALMODE);
	}
 ...
 *   d2_bf_colorize2 - bitmap colors are interpolated between color register index 0 and 1 (see:
 * <d2_setcolor>) d2_bf_usealpha - alpha value from bitmap is used d2_bf_invertalpha - alpha value
 * from bitmap is inverted before use (requires usealpha) d2_bf_no_blitctxbackup   - for this blit
 * don't backup context data for better performance; previous texture modes get lost and must be set
 * again
 *
 *
 * (start code)
 ...
 *
 * note:
 * Wrapping will work with source bitmap dimensions that are integer powers of two only
 * (2,4,8,16,32,..)!
 *
 * texture pitch can be >= 2048 if (srcheight-1) * pitch < 2048*1024 and if srcheight is multiple of
 * dstheight
 *
		   ...
		   d2_point dsty, d2_u32 flags)
{
	...
	D2_VALIDATE(srcwidth > 0, D2_INVALIDWIDTH);
	...
	D2_VALIDATEP(ctx->blit_src, D2_NULLPOINTER);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	D2_VALIDATE(!((0 != (d2_tm_wrapu & flags)) &&
		      (0 != ((d2_u32)ctx->blit_width & ((d2_u32)ctx->blit_width - 1)))),
		    D2_INVALIDWIDTH);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_VALIDATE(!((0 != (d2_tm_wrapv & flags)) &&
		      (0 != ((d2_u32)ctx->blit_height & ((d2_u32)ctx->blit_height - 1)))),
		    D2_INVALIDHEIGHT);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	D2_VALIDATE(!((0 != (d2_tm_wrapu & flags)) &&
		...
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_VALIDATE(!((0 != (d2_tm_wrapv & flags)) &&
		      (0 != ((d2_u32)texHeight & ((d2_u32)texHeight - 1)))),
		    D2_INVALIDHEIGHT);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	if (0 != isRLE) {
		(void)d2_settexture(handle, src, ctx->blit_pitch, ctx->blit_width, ctx->blit_height,
				    ctx->blit_format);
	} else {
		(void)d2_settexture(handle, src, ctx->blit_pitch, texWidth, texHeight,
				    ctx->blit_format);
	}
	...
	if (0 != (flags & d2_bf_usealpha)) {
		if (0 != (flags & d2_bf_invertalpha)) {
			...
			texmodecl1 = 0xff000000u; /* multiply by ctx->constalpha happens in
						     d2_calctexturealpha_intern; */
			texmodecl2 = 0x00000000u;
		} else {
			/* alpha multiply */
			texmodecl1 = 0x00000000u;
			texmodecl2 = 0xff000000u; /* multiply by ctx->constalpha happens in
						     d2_calctexturealpha_intern; */
		}
	} else {
		/* alpha replace */
	...
	if ((d2_u16)dstwidth > D2_FIX4(1)) {
		dxu = (d2_s32)(((d2_u32)D2_FIX16(sw)) / ((d2_u16)dstwidth));
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		u0 = dxu /
		     2; /* offset by half a pixel step, as we sample in the center of our pixels */
	}
	if (dstheight > D2_FIX4(1)) {
		dyv = (d2_s32)(((d2_u32)D2_FIX16(sh)) / ((d2_u16)dstheight));
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		v0 = dyv /
		     2; /* offset by half a pixel step, as we sample in the center of our pixels */
	}
	...
	if (0 != (flags & d2_bf_mirroru)) {
		dxu = -dxu;
		u0 = D2_FIX16(srcwidth) - u0;
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
	}
	if (0 != (flags & d2_bf_mirrorv)) {
		dyv = -dyv;
		v0 = D2_FIX16(srcheight) - v0;
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
	}

	/* filtering: offset by -1/2 texel compared to nearest neighbour, to position the 2x2 kernel
	 * correctly */
	...
	if (0 != isRLE) {
		u0 += D2_FIX16((d2_s32)srcx);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		v0 += D2_FIX16((d2_s32)srcy);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
	}
	...
	/* do blit */
	(void)d2_renderbox_intern((d2_devicedata *)handle, blitcontext, dstx, dsty, dstwidth,
				  dstheight);

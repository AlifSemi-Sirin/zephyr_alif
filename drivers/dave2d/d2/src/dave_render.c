...
static D2_INLINE void d2_checkwaitpipeline_intern(d2_devicedata *handle, d2_u32 cr2)
{
	d2_u32 cr2_blend_wait_mask = D2C_WRITEALPHA2 | D2C_WRITEALPHA1 | D2C_BDFA | D2C_BDIA |
				     D2C_BSFA | D2C_BSIA | D2C_USE_ACB | D2C_WRITEFORMAT3 |
				     D2C_WRITEFORMAT2 | D2C_WRITEFORMAT1;
			    ...
			    const d2_u32 *flags, d2_u32 count)
{
	...
	px1 = vert[0];
	py1 = vert[1]; /* center point */
	px2 = vert[2];
	py2 = vert[3]; /* first support point */
...
void d2_setupmaterial_intern(d2_devicedata *handle, d2_contextdata *ctx)
{
	...
	ctx->internaldirty &= (d2_u8) ~((d2_u8)d2_dirty_material);
		/* PRQA S 4130 */ /* $Misra: #MISRA_BUG_UNSIGNED_BITWISE_NOT $*/

	if (ctx->alphablendflags == d2_blendf_blendcolor2) {
		ctx->cr2mask &=
			~(D2C_WRITEALPHA2 | D2C_WRITEALPHA1); /* 00 = use alpha from color2 */
	...
	case d2_fm_twocolor:
		...
		if (ctx->alphablendflags == d2_blendf_blendcolor2) {
			D2_DLISTWRITEU(D2_COLOR2,
				       (ctx->basecolor[1] | ((d2_u32)ctx->basealpha[1] << 24)));
		} else {
	...
	case d2_fm_texture:
		...
		if (ctx->alphablendflags == d2_blendf_blendcolor2) {
			D2_DLISTWRITEU(D2_COLOR2, ((ctx->texcolreg[1] & 0xffffffu) |
						   (((d2_u32)ctx->basealpha[1]) << 24)));
		} else {
			D2_DLISTWRITEU(D2_COLOR2, ctx->texcolreg[1]);
		}

		/* in case of RLE wait for pipeline complete before setting new texture origin */
		if ((0 != (ctx->rlemask & D2C_RLE_ENABLE)) ||
		    (0 != D2_DEV(handle)->lasttextwasrle)) {
		...
		cr2_value = D2C_TEXTUREENABLE | ctx->cr2mask | ctx->texmodemask | ctx->rlemask |
			    ctx->clutmask | ctx->colkeymask;
...
void d2_startrender_intern(d2_devicedata *handle, const d2_bbox *bbox, d2_u32 delay)
{
	...
	/* find size */
	w = D2_INT4(bbox->xmax - bbox->xmin) + 1;
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	h = D2_INT4(bbox->ymax - bbox->ymin) + 1;
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/

	/* set registers */
	D2_DLISTWRITEU(D2_PITCH,
		       (((d2_u16)handle->pitch) +
			(((d2_u16)delay) *
			 65536u))); /*note : and can be avoided if pitch forced to be >0 */
	D2_DLISTWRITES(D2_SIZE, (h * 65536) + w);

	/* start rendering */
	switch (handle->bpp) {
	case 1:
		D2_DLISTWRITEU(D2_ORIGIN, (((d2_s8 *)handle->framebuffer) + D2_INT4(bbox->xmin) +
					   (D2_INT4(bbox->ymin) * handle->pitch)));
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		break;

	case 2:
		D2_DLISTWRITEU(D2_ORIGIN, (((d2_s16 *)handle->framebuffer) + D2_INT4(bbox->xmin) +
					   (D2_INT4(bbox->ymin) * handle->pitch)));
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		break;

	case 4:
		D2_DLISTWRITEU(D2_ORIGIN, (((d2_s32 *)handle->framebuffer) + D2_INT4(bbox->xmin) +
					   (D2_INT4(bbox->ymin) * handle->pitch)));
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		break;
...
void d2_startrender_bottom_intern(d2_devicedata *handle, const d2_bbox *bbox, d2_u32 delay)
{
	...
	/* set registers */
	D2_DLISTWRITES(D2_PITCH,
		       (((d2_s32)((d2_u16)(-handle->pitch)))) + (((d2_s16)delay) * 65536));
	D2_DLISTWRITES(D2_SIZE, (h * 65536) + w);
	...
	case 1:
		D2_DLISTWRITEU(D2_ORIGIN, (((d2_s8 *)handle->framebuffer) + D2_INT4(bbox->xmin) +
					   (D2_INT4(bbox->ymax) * handle->pitch)));
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		break;
...
d2_s32 d2_renderbox(d2_device *handle, d2_point x1, d2_point y1, d2_width w, d2_width h)
{
	...
	case d2_rm_solid:
		err = d2_renderbox_solid(handle, x1, y1, w, h);
		break;
	case d2_rm_outline:
		err = d2_renderbox_outline(handle, x1, y1, w, h);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderbox_solidoutline(handle, x1, y1, w, h);
		break;
	case d2_rm_shadow:
		err = d2_renderbox_shadow(handle, x1, y1, w, h);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderbox_solidshadow(handle, x1, y1, w, h);
		break;
	case d2_rm_postprocess:
		err = d2_renderbox_solid(handle, x1, y1, w, h);
		break;
		     ...
		     d2_width w, d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderline_solid(handle, x1, y1, x2, y2, w, flags);
		break;
	case d2_rm_outline:
		err = d2_renderline_outline(handle, x1, y1, x2, y2, w, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderline_solidoutline(handle, x1, y1, x2, y2, w, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderline_shadow(handle, x1, y1, x2, y2, w, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderline_solidshadow(handle, x1, y1, x2, y2, w, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderline_solid(handle, x1, y1, x2, y2, w, flags);
		break;
		      ...
		      d2_width w1, d2_width w2, d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderline2_solid(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
	case d2_rm_outline:
		err = d2_renderline2_outline(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderline2_solidoutline(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderline2_shadow(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderline2_solidshadow(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderline2_solid(handle, x1, y1, x2, y2, w1, w2, flags);
		break;
		    ...
		    d2_point x3, d2_point y3, d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_rendertri_solid(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
	case d2_rm_outline:
		err = d2_rendertri_outline(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_rendertri_solidoutline(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
	case d2_rm_shadow:
		err = d2_rendertri_shadow(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_rendertri_solidshadow(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
	case d2_rm_postprocess:
		err = d2_rendertri_solid(handle, x1, y1, x2, y2, x3, y3, flags);
		break;
		     ...
		     d2_point x3, d2_point y3, d2_point x4, d2_point y4, d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderquad_solid(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
	case d2_rm_outline:
		err = d2_renderquad_outline(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderquad_solidoutline(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderquad_shadow(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderquad_solidshadow(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderquad_solid(handle, x1, y1, x2, y2, x3, y3, x4, y4, flags);
		break;
...
d2_s32 d2_rendercircle(d2_device *handle, d2_point x, d2_point y, d2_width r, d2_width w)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_rendercircle_solid(handle, x, y, r, w);
		break;
	case d2_rm_outline:
		err = d2_rendercircle_outline(handle, x, y, r, w);
		break;
	case d2_rm_solid_outlined:
		err = d2_rendercircle_solidoutline(handle, x, y, r, w);
		break;
	case d2_rm_shadow:
		err = d2_rendercircle_shadow(handle, x, y, r, w);
		break;
	case d2_rm_solid_shadow:
		err = d2_rendercircle_solidshadow(handle, x, y, r, w);
		break;
	case d2_rm_postprocess:
		err = d2_rendercircle_solid(handle, x, y, r, w);
		break;
 ...
 *
 * remarks:
 *   The figure below shows four examples of different wedges and the corresponding normal vectors
 * (n1, n2). In order to get correct antialiasing all vectors must be normalized to a length of one.
 *   (see wedge.png)
 *
 * example:
 * (start code)
 ...
 * ...
 * d2_setcolor(handle, 0, 0x0000ff);
 * d2_renderwedge(handle, 100<<4, 100<<4, 50<<4, 0,    1<<16, 0<<16,   -46340, -46340,
 * d2_wf_concave); d2_renderwedge(handle, 200<<4, 100<<4, 50<<4, 0,    1<<16, 0<<16,    0<<16,
 * -1<<16,   d2_wf_concave); d2_renderwedge(handle, 300<<4, 100<<4, 50<<4, 0,    1<<16, 0<<16,
 * 46340, -46340,   d2_wf_concave); d2_renderwedge(handle, 400<<4, 100<<4, 50<<4, 0,    1<<16,
		      ...
		      d2_s32 ny1, d2_s32 nx2, d2_s32 ny2, d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderwedge_solid(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
	case d2_rm_outline:
		err = d2_renderwedge_outline(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderwedge_solidoutline(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderwedge_shadow(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderwedge_solidshadow(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderwedge_solid(handle, x, y, r, w, nx1, ny1, nx2, ny2, flags);
		break;
			 ...
			 d2_u32 flags)
{
	...
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(count > 0, D2_VALUENEGATIVE);
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderpolyline_solid(handle, data, count, w, flags);
		break;
	case d2_rm_outline:
		err = d2_renderpolyline_outline(handle, data, count, w, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderpolyline_solidoutline(handle, data, count, w, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderpolyline_shadow(handle, data, count, w, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderpolyline_solidshadow(handle, data, count, w, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderpolyline_solid(handle, data, count, w, flags);
		break;
			  ...
			  d2_u32 flags)
{
	...
	switch (D2_DEV(handle)->rendermode) {
	case d2_rm_solid:
		err = d2_renderpolyline2_solid(handle, data, count, w, flags);
		break;
	case d2_rm_outline:
		err = d2_renderpolyline2_outline(handle, data, count, w, flags);
		break;
	case d2_rm_solid_outlined:
		err = d2_renderpolyline2_solidoutline(handle, data, count, w, flags);
		break;
	case d2_rm_shadow:
		err = d2_renderpolyline2_shadow(handle, data, count, w, flags);
		break;
	case d2_rm_solid_shadow:
		err = d2_renderpolyline2_solidshadow(handle, data, count, w, flags);
		break;
	case d2_rm_postprocess:
		err = d2_renderpolyline2_solid(handle, data, count, w, flags);
		break;
...
d2_s32 d2_rendertrifan(d2_device *handle, const d2_point *data, const d2_u32 *flags, d2_u32 count)
{
	...
	if (0 != (rmode & d2_rm_outline)) {
		(void)d2_renderpolyline_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, data,
					       NULL, count + 2, D2_DEV(handle)->outlinewidth,
					       d2_le_closed, 0, 0);
	}

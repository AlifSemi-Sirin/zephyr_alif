...
void d2_circlesetup_intern(d2_devicedata *handle, const d2_contextdata *ctx, d2_u32 index,
			   d2_point x, d2_point y, d2_width r, d2_s32 band, d2_s32 invert,
			   d2_s32 hiprec)
{
	d2_s32 a, b, f, ir;
	d2_u32 ib;
	...
	if (0 != (ctx->features & d2_feat_blur)) {
		if ((0 == hiprec) ||
			...
			ir = (((-1 * D2_FIX16(16)) * D2_FIX4(1)) * (D2_FIX4(1) / 2)) /
			     (r * blur); /* 1 / 2rb */
#ifndef _NO_LL_
			ir64 = (((((-1 * (d2_int64)D2_FIX16(16 << 0)) * D2_FIX4(1)) *
				  (D2_FIX4(1) / 2)) /
				 (d2_int64)r) *
				(1 << 8)) /
			       (d2_int64)blur; /* 1 / 2rb */
#else
			d2_cast32to64((((-1 * D2_FIX16(16 << 0)) * D2_FIX4(1)) * (D2_FIX4(1) / 2)) /
					      r,
				      &ir64);
			...
			f = (((((xr * xr) + (yr * yr)) / (1 << 8)) * ir) / (1 << 4)) -
			    ((((r / 2) + (xr + yr)) * (d2_s32)ib) / (1 << 4)); /* (x^2+y^2-r^2) * ir */

#ifndef _NO_LL_
			f64 = (((((xr * xr) + (yr * yr)) / (1 << 8)) * (d2_int64)ir64) / (1 << 4)) -
			      ((((r / 2) + (xr + yr)) * (d2_int64)ib) *
			       (1 << 4)); /* (x^2+y^2-r^2) * ir */
			f = (d2_s32)(f64 / (1 << 8));
#else
			d2_mul3264((((xr * xr) + (yr * yr)) / (1 << 8)), &ir64, &tmp);
			d2_shiftright64(&tmp, 4, &f64);
			d2_mul3232to64(((r / 2) + (xr + yr)), (d2_s32)ib, &tmp);
			d2_shiftleft64(&tmp, 4, &tmp2);
			d2_sub64(&f64, &tmp2, &tmp); /* (x^2+y^2-r^2) * ir */
			...
			/* calc circle parameters (16:16 from 10:4) */
			ir = (((((-1 * D2_FIX16(16 << 0)) * D2_FIX4(1)) * (D2_FIX4(1) / 2)) / r) *
			      (1 << LIMITER_HIPRECISION)) /
			     blur; /* 1 / 2rb */
#ifndef _NO_LL_
			f64 = (((((xr * xr) + (yr * yr)) / (1 << 8)) * (d2_int64)ir) / (1 << 4)) -
			      ((((r / 2) + (xr + yr)) * (d2_int64)ib) *
			       (1 << (LIMITER_HIPRECISION - 4))); /* (x^2+y^2-r^2) * ir */
#else
			...
			ir >>= 4; /* works */ /* PRQA S 0502 */             /* $Misra:
									       #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		...
		if (0 == (xr | yr)) /* PRQA S 4130 */ /* $Misra: #PERF_BITWISE $*/
		{
			...
			ir = ((-1 * D2_FIX16(1 << TWEAK)) * (D2_FIX4(1) / 2)) / r;
			xq = xr * xr;
			yq = yr * yr;

#ifndef _NO_LL_
			/* (note) using the '/' variant of the following line will actually call a
			 * 64bit division subroutine the result of it sometimes differs by one bit
			 * compared to the '>>' variant, making "test_renderpolyline2" fail.
			 *         however, this codepath is only taking in the windows softdave
			 * emulation environment
			 */
			ft = ((xq + yq) * (d2_int64)ir) >> (8 + TWEAK);           /* works */
			/*ft = ((xq + yq) * (d2_int64)ir) / (1 << (8 + TWEAK));*/ /* test_renderpolyline2
										     fails */
			/* ftXXX = ((xq + yq) * (d2_int64)ir) / (1 << (8 + TWEAK));*/ /* test_renderpolyline2
											 fails */

 ...
 * (start code)
 * d2_startframe(handle); // --> start HW rendering of the previous frame (buffer a), switch to
 *buffer b for all following render commands
 *
 * // ... now issue new render commands to buffer b while HW is rendering the previous frame from
 *render buffer a ...
 *
 * d2_endframe(handle); // --> close render buffer b which has just captured all render commands,
 *wait for HW to finish rendering of buffer a
 *
	...
	if (NULL != rbuffer) {
		...
		if (0 != d2_initdlist_intern(handle, &rbuffer->baselist, initialsize)) {
			if (0 == D2_DEV(handle)->lowlocalmem_mode) {
				...
				if (0 == d2_initrblayer_intern(handle, &rbuffer->layer[0],
							       D2_DLISTSCRATCH)) {
					bOOM = 1;
				}
			}
...
d2_s32 d2_freerenderbuffer(d2_device *handle, d2_renderbuffer *buffer)
{
	...
	D2_CHECKERR(D2_DRB(buffer)->baselist.device == D2_DEV(handle), D2_INVALIDDEVICE);
		/* PRQA S 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	if ((D2_DEV(handle)->renderbuffer[0] == D2_DRB(buffer)) ||
 ...
 * involves flushing of scratch buffers, possibly merge of layers in certain render modes (see
 * <d2_selectrendermode>) and potentially copying the display list to a dedicated video memory or at
 * least flushing of the CPU data caches. The buffer passed to d2_executerenderbuffer can be used
 * for execution again by calling d2_executerenderbuffer again with the same buffer as argument. But
 * before new render commands can be written to the buffer it must be selected by
 * <d2_selectrenderbuffer>.
 *
 * Note that before calling d2_executerenderbuffer, it must be made sure that no render buffer is
 * currently executed on the hardware. This can be done by either waiting for the respective
 * interrupt or by calling <d2_flushframe>.
 *
 ...
 *
 * Note that calling this function from inside an interrupt service routine is not recommended, as
 * the execution time can not be foreseen. If it is called from inside the D/AVE 'display list
 * finished' ISR anyway, it needs to be made sure that clearing the IRQ happens before the call to
 * d2_executerenderbuffer! Register writes to the D/AVE core are not allowed while a display list is
 * being executed.
 *
 * Note that if d2_ef_execute_once is used a new render buffer must be selected by
 * <d2_selectrenderbuffer> even if it is the same render buffer.
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
 *   buffer - renderbuffer address
	...
	while (NULL != blk) {
				 ...
				 *)((d2_s32 *)entry +
				    (blk->quantity *
				     (sizeof(d2_dlist_entry) /
				      sizeof(d2_s32)))); /* step through all entries in the block */
		...
		while (entry < lastEntry) {
			...
			if (0 != (adrmask & 0x80808080u)) {
					...
					if (0xffu == (adrmask & 0xffu)) {
						     ...
						     (argument & 4u))) /* special bit 1 or 2 set? */
						{
							...
							entry = (d2_dlist_entry *)((d2_s32 *)entry +
										   1);
							/*continue;*/
						} else { /* special bit 0 must be set */
							/* terminate */
				...
				if (D2_ORIGIN == (adrmask & 0xffu)) {
					entry->value[0] =
						(d2_s32)(((d2_s8 *)entry->value[0] - oldPtr) +
							 (d2_s8 *)ptr);
				}

				if ((D2_ORIGIN << 8) == (adrmask & 0x0000ff00u)) {
					entry->value[1] =
						(d2_s32)(((d2_s8 *)entry->value[1] - oldPtr) +
							 (d2_s8 *)ptr);
				}

				if ((D2_ORIGIN << 16) == (adrmask & 0x00ff0000u)) {
					entry->value[2] =
						(d2_s32)(((d2_s8 *)entry->value[2] - oldPtr) +
							 (d2_s8 *)ptr);
				}

				if ((D2_ORIGIN << 24) == (adrmask & 0xff000000u)) {
					entry->value[3] =
						(d2_s32)(((d2_s8 *)entry->value[3] - oldPtr) +
							 (d2_s8 *)ptr);
				}
 ...
 *
 * This function can only be used when double word clearing is
 * disabled (see: <d2_opendevice>) and 'low localmem' mode is not enabled (see:
 * <d2_lowlocalmemmode>).
 *
 * note:
 * Use <d2_layermerge> to take care about outline and solid parts.
 ...
 * render buffer.
 *
 * This function can only be used when 'low localmem' mode is not enabled (see:
 * <d2_lowlocalmemmode>).
 *
 * note:
 *   If <d2_adddlist> (d2_al_no_copy) commands have been used these added
...
static void d2_scratchgrowlayer_intern(d2_device *handle)
{
	...
	d2_dev->dlscratch_pos = (d2_dev->dlscratch_pos - d2_dev->dlscratch_base) +
				d2_dev->selectedbuffer->layer[0].scratch;

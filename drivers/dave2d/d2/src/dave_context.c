 ...
 * A device handle (see: <Device management>) is the root of all process dependant data and is
 *passed as first parameter to every function. This construct allows the driver functions to be
 *reentrant in case of a multiple drawing engine units.
 *
 ...
 *
 *  selected context - This one is changed (written to) by the set commands shown below (see:
 *<d2_selectcontext>) solid context - Used as a source (read from) when rendering interior regions
 *(see: <d2_solidcontext>) outline context - Used as a source (read from) when rendering outlines or
 *shadows (see: <d2_outlinecontext>)
 *
 * All three (selected, solid and outline) can point to the same context.
 * Every driver function that starts with *get* or *set* will work on the currently selected context
 *(with the obvious exception of geterror and getversion).
 *
...
d2_context *d2_newcontext(d2_device *handle)
{
	...
	ctx->texmodemask =
		D2C_TEXTUREFILTERX | D2C_TEXTUREFILTERY | D2C_TEXTURECLAMPX | D2C_TEXTURECLAMPY;
	ctx->texbpp = 4;
	ctx->tbstylemask = D2C_READFORMAT2;
	ctx->rlemask = 0;
	ctx->clutmask = 0;
	ctx->colkeymask = 0;
	ctx->cr2mask = D2_DEV(handle)->fbstylemask | ctx->alphablendmask | ctx->blendmask |
		       ctx->tbstylemask | ctx->rlemask | ctx->clutmask | ctx->colkeymask;
	ctx->gradients = 0;
...
{
	D2_VALIDATEP(handle, D2_INVALIDDEVICE); /* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERRP((((d2_u32)mode) < 4U), D2_INVALIDENUM);
 ...
 * Set color registers.
 *
 * Set one of the two color registers, color1 and color2. Second register (index 1) is used for
 * pattern rendering and during d2_fm_twocolor fillmode only.
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
 ...
 *
 * Blendmode defines how the RGB channels of new pixels (source) are combined with already existing
 * data in the framebuffer (destination). Blend modes for alpha channel are set by
 ...
 *
 * note:
 *  If the framebuffer format is set to d2_mode_alpha8 (<d2_framebuffer>) then alpha blend mode
 * should be set to write src alpha. For d2_mode_alpha8 the blend modes set by <d2_setblendmode> do
 * apply.
 *
 * note:
 *   the blend flags of the selected context is set to d2_blendf_default (see:
 ...
 *     d2_blendf_blendcolor2 - use color2 alpha as dst alpha as input for the blending formula (see:
 * <d2_setalphaex>)
 *
 * note:
 *  If the framebuffer format is set to d2_mode_alpha8 (<d2_framebuffer>) the alpha blend mode
 * should be set to write src alpha: For d2_mode_alpha8 the blend modes set by <d2_setblendmode> do
 * apply.
 *
 *  On older D/AVE 2D hardware revisions without alpha channel blending (see: <d2_getrevisionhw>,
 * D2FB_ALPHACHANNELBLENDING) only the following two blend combinations can be used for the alpha
 * channel:
			      ...
			      d2_u32 blendflags)
{
	...
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR((((d2_u32)blendflags) < 256), D2_INVALIDENUM);
		/* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
...
d2_s32 d2_setblur(d2_device *handle, d2_width blur)
{
	...
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(blur != 0, D2_VALUETOOSMALL);
		/* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
 ...
 * when the two segments meet at 180 deg.
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
...
d2_s32 d2_setpatternsize(d2_device *handle, d2_s32 size)
{
	...
	D2_CHECKERR(size <= (D2_DEV(handle)->ctxselected->device->maxpatlen), D2_INVALIDWIDTH);
		/* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
 ...
 *   x,y - startpoint of gradient (point of alpha 0) (fixedpoint)
 *   nx,ny - normal vector of the clipping edge, pointing towards the non-clipped side (16.16
 * fixedpoint) flags - reserved
 *
 * returns:
 *   errorcode (D2_OK if successful) see list of <Errorcodes> for details
 *
...
d2_s32 d2_setcircleextend(d2_device *handle, d2_width offset)
{
	...
	D2_VALIDATE(handle, D2_INVALIDDEVICE); /* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/
	D2_CHECKERR(offset >= 0, D2_VALUETOOSMALL);
		/* PRQA S 3112, 4130 */ /* $Misra: #DEBUG_MACRO $*/

 ...
 *  2016-06-29 MHo  bugfix in fbblitcopy: handle SRC below DST in case of overlapping rects
 * correctly in presence of an asymmetric clip rect 2017-07-27 HFu  bugfix in fbblitcopy: corrected
 * wait-insert to full wait instead of pipeline only
 */
...
d2_s32 d2_utility_maptriangle(d2_device *handle, const d2_f32 *points, const d2_f32 *uvs)
{
	...
	d = ((points[0 * 2] - points[2 * 2]) * (points[(1 * 2) + 1] - points[(2 * 2) + 1])) -
	    ((points[1 * 2] - points[2 * 2]) * (points[(0 * 2) + 1] - points[(2 * 2) + 1]));
	/* validate. if d==0 use dxyuv=0 [no area triangle] */
	dx = tw / d;
	dy = th / d;

	dxu = (d2_s32)((((uvs[0 * 2] - uvs[2 * 2]) * (points[(1 * 2) + 1] - points[(2 * 2) + 1])) -
			((uvs[1 * 2] - uvs[2 * 2]) * (points[(0 * 2) + 1] - points[(2 * 2) + 1]))) *
		       dx);

	dxv = (d2_s32)((((uvs[(0 * 2) + 1] - uvs[(2 * 2) + 1]) *
			 (points[(1 * 2) + 1] - points[(2 * 2) + 1])) -
			((uvs[(1 * 2) + 1] - uvs[(2 * 2) + 1]) *
			 (points[(0 * 2) + 1] - points[(2 * 2) + 1]))) *
		       dy);

	dyu = (d2_s32)((((uvs[1 * 2] - uvs[2 * 2]) * (points[0 * 2] - points[2 * 2])) -
			((uvs[0 * 2] - uvs[2 * 2]) * (points[1 * 2] - points[2 * 2]))) *
		       dx);

	dyv = (d2_s32)((((uvs[(1 * 2) + 1] - uvs[(2 * 2) + 1]) * (points[0 * 2] - points[2 * 2])) -
			((uvs[(0 * 2) + 1] - uvs[(2 * 2) + 1]) * (points[1 * 2] - points[2 * 2]))) *
		       dy);

	return d2_settexturemapping(handle, (d2_point)(points[0] * 16), (d2_point)(points[1] * 16),
				    (d2_s32)(uvs[0] * tw), (d2_s32)(uvs[1] * th), dxu, dyu, dxv,
				    dyv);
}
				  ...
				  d2_s16 dsty, d2_u16 wt)
{
	...
	for (y = 0; y < (d2_u32)dstheight; y++) {
		...
		result = d2_renderbox(handle, (d2_point)(dstx << 4),
				      (d2_point)((dsty + (d2_s16)y) << 4),
 ...
 *
 * This is a wrapper function around <d2_blitcopy>, which copies a rectangular region inside a
 * single buffer. Source and destination buffer are the current framebuffer, set via
 * <d2_framebuffer>. This function handles the different cases of potentially overlapping source and
 * destination areas and makes sure no read-after-write problems arise. Note that both source and
 * destination rectangles need to lie fully inside the framebuffer for this function to work
 * correctly.
 *
 * parameters:
 *   handle - device pointer (see: <d2_opendevice>)
 ...
 *   d2_bf_no_blitctxbackup - for this blit don't backup context data for better performance;
 * previous texture modes get lost and must be set again
 *
 * returns:
			     ...
			     d2_blitpos srcy, d2_blitpos dstx, d2_blitpos dsty, d2_u32 flags)
{
	...
	D2_VALIDATE(height > 0, D2_INVALIDHEIGHT);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_VALIDATE(width <= 1024, D2_INVALIDWIDTH);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	D2_VALIDATE(height <= 1024, D2_INVALIDHEIGHT);
		/* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
	...
	/* since we now read the framebuffer as texture, we need to make sure everything has been
	 * flushed first */
	...
	if ((dsty > srcy) && (dsty < (srcy + height)) && ((dstx + width) > srcx) &&
		...
		D2_DEV(handle)->clipymax =
			(d2_point)((d2_s16)(D2_FIX4(fb_height - 1)) -
				   clip_ymin); /* MHO: once the framebuffer is mirrored temporarily,
						  any coordinates */
		D2_DEV(handle)->clipymin =
			(d2_point)((d2_s16)(D2_FIX4(fb_height - 1)) -
				   clip_ymax); /* MHO: ..of significance in that time, need to be
						  mirrored internally, too */
		flags ^= d2_bf_mirrorv;
		flipped = 1;
	}

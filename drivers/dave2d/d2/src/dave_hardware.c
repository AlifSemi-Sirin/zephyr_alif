...
d1_device *d2hw_acquire(d2_device *handle, d2_u32 flags)
{
	...
	if (NULL != d1handle) {
		D2_DEV(handle)->hwrevision =
			(d2_u32)d1_getregister(d1handle, D1_DAVE2D, D2_HWREVISION);
		...
		/* attention: don't change cache settings while HW is running -> added caution note
		 * in doc of d2_opendevice() */
		d1_setregister(d1handle, D1_DAVE2D, D2_CACHECTL,
			       (d2_slong)D2_DEV(handle)->cachectlmask);
...
void d2hw_finish(const d2_device *handle)
{
	...
	if (0 != (D2_DEV(handle)->flags & d2_df_no_dlist)) {
		...
		while (0 != ((d2_u32)d1_getregister(hwId, D1_DAVE2D, D2_STATUS) &
			     (D2C_BUSY_ENUM | D2C_BUSY_WRITE))) {
		}
	} else {
		/* wait for dlist finish signal */
		while (0 !=
		       ((d2_u32)d1_getregister(hwId, D1_DAVE2D, D2_STATUS) & D2C_DLISTACTIVE)) {

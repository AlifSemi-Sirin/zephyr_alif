			...
			if (0 == yas) {
				...
				D2_DLISTWRITES(D2_L1START,
					       (d2_s32)(((((d2_u32)(bbox.xmin - x1)) << (16 - 4)) +
							 D2_FIX16(1))));
				D2_DLISTWRITES(D2_L1XADD, D2_FIX16(1));
					/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
				D2_DLISTWRITES(D2_L1YADD, D2_FIX16(0));

				D2_DLISTWRITES(D2_L2START,
					       (d2_s32)(((((d2_u32)(x2 - bbox.xmin)) << (16 - 4)) +
							 D2_FIX16(1))));
				D2_DLISTWRITES(D2_L2XADD, D2_FIX16(-1));
					/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
				...
				D2_DLISTWRITES(D2_L3YADD, D2_FIX16(1));
					/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
				...
				D2_DLISTWRITES(D2_L4YADD, D2_FIX16(-1));
					/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
...
static void d2_clearbox_intern(d2_devicedata *handle, const d2_contextdata *ctx, d2_color clearcol)
{
	...
	if (((handle->flags & d2_df_no_dwclear) == 0) && (handle->bpp < 4) &&
	    (((handle->pitch / s) * s) ==
	     handle->pitch) && /* pitch must be divisible by s without remainder */
	    (((d2_u32)handle->framebuffer & (~3u)) ==
	     (d2_u32)handle->framebuffer) &&                  /* fb start must be divisible by 4 */
		      ...
		      1))) /* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	{
		...
		case d2_mode_alpha8:
			...
			dblcolor =
				(clearcol << 24) | (clearcol << 16) | (clearcol << 8) | (clearcol);
			break;

		case d2_mode_argb4444:
		case d2_mode_rgb444:
			clearcol =
				(((clearcol & 0xf0000000u) >> 16) |
				 ((clearcol & 0x00f00000u) >> 12) |
				 ((clearcol & 0x0000f000u) >> 8) | ((clearcol & 0x000000f0u) >> 4));
			dblcolor = (clearcol << 16) | (clearcol);
			break;

		case d2_mode_rgba4444:
			clearcol =
				(((clearcol & 0xf0000000u) >> 28) |
				 ((clearcol & 0x00f00000u) >> 8) | ((clearcol & 0x0000f000u) >> 4) |
		...
		case d2_mode_rgb565:
			clearcol =
				(((clearcol & 0x00f80000u) >> 8) | ((clearcol & 0x0000fc00u) >> 5) |
		...
		D2_DLISTWRITES(
			D2_CONTROL2,
			D2C_WRITEFORMAT2 | D2C_BDI | D2C_WRITEALPHA1 |
				D2C_RLE_ENABLE); /* D2C_RLE_ENABLE seems irrelevant here but is
						    necessary for a HW issue: Mantis #1416 */
		...
		switch (handle->bpp) {
		case 1:
			D2_DLISTWRITES(D2_ORIGIN, (d2_s32)(((d2_s8 *)handle->framebuffer) +
							   D2_INT4(bbox.xmin) +
		...
		case 2:
			D2_DLISTWRITES(D2_ORIGIN, (d2_s32)(((d2_s16 *)handle->framebuffer) +
							   D2_INT4(bbox.xmin) +
							   (D2_INT4(bbox.ymin) * handle->pitch)));
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
			break;
		...
		case 4:
			D2_DLISTWRITES(D2_ORIGIN, (d2_s32)(((d2_s32 *)handle->framebuffer) +
							   D2_INT4(bbox.xmin) +
							   (D2_INT4(bbox.ymin) * handle->pitch)));
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
			break;
		...
		D2_DLISTWRITES(
			D2_CONTROL2,
			(d2_s32)(D2_DEV(handle)->fbstylemask | D2C_BDI | D2C_WRITEALPHA1 |
				 D2C_RLE_ENABLE)); /* D2C_RLE_ENABLE seems irrelevant here but is
						      necessary for a HW issue: Mantis #1416 */
		d2_startrender_intern(handle, &bbox, 0);
		(void)d2_insertwaitpipedlist_intern(handle);
...
d2_s32 d2_renderbox_shadow(d2_device *handle, d2_point x1, d2_point y1, d2_width w, d2_width h)
{
	(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxoutline,
				  (d2_point)(D2_DEV(handle)->soffx + x1),
				  (d2_point)(D2_DEV(handle)->soffy + y1), w, h);
...
d2_s32 d2_renderbox_outline(d2_device *handle, d2_point x1, d2_point y1, d2_width w, d2_width h)
{
	...
	if (wo >= D2_EPSILON) {
		...
		if ((wo == D2_FIX4(1)) ||
		    ((ctx->linejoin == d2_lj_miter) && (ctx->miterlimit > wo) &&
		     (0 == (ctx->features & d2_feat_blur)))) {
			/* render outline with thin boxs (optimal performance) */
			(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxoutline,
						  (d2_point)(x1 - wo), (d2_point)(y1 - wo), wo,
						  (d2_width)(h + (2 * wo))); /*left */
			(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x1,
						  (d2_point)(y1 - wo), w, wo); /*top */
			(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxoutline,
						  (d2_point)(x1 + w), (d2_point)(y1 - wo), wo,
						  (d2_width)(h + (2 * wo))); /*right */
			(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x1,
						  (d2_point)(y1 + h), w, wo); /*bottom */
		} else {
			/* render outline as polyline (supports linejoins) */
			verts[0] = (d2_point)((x1 - woh) - 8);
			verts[1] =
				(d2_point)((y1 - woh) -
					   8); /* (note) parentheses added for MISRA compliance.. */
			verts[2] = (d2_point)((x1 + w) + (woh - 8));
			verts[3] = (d2_point)((y1 - woh) - 8);
			verts[4] = (d2_point)((x1 + w) + (woh - 8));
			verts[5] = (d2_point)((y1 + h) + (woh - 8));
			verts[6] = (d2_point)((x1 - woh) - 8);
			verts[7] = (d2_point)((y1 + h) + (woh - 8));

			(void)d2_renderpolyline_intern(D2_DEV(handle), ctx, verts, NULL, 4, wo,
						       d2_le_closed, 0, 0);
		}
	}
				 ...
				 d2_width h)
{
	...
	(void)d2_renderbox_inline(D2_DEV(handle), D2_DEV(handle)->ctxsolid, (d2_point)(x1 - 16),
				  (d2_point)(y1 - 16), (d2_width)(w + 32), (d2_width)(h + 32));

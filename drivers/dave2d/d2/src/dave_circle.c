			      ...
			      d2_width r, d2_width w)
{
	...
	if (ctx->circleextendoffset > 0) {
		...
		/* bbox_enlarge = (bbox_enlarge > ((256 * 128) - 1)) ? ((256 * 128) - 1) :
		 * bbox_enlarge; */ /* (note) dead code since bbox_enlarge is a 16bit signed integer */
	...
	if (0 == w) {
		...
		control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_SPANABORT | D2C_SPANSTORE |
		...
		if ((r > (w * w)) && (0 == (ctx->features & d2_feat_blur))) {
			...
			/* set register values (geometric parameters) */
			d2_circlesetup_intern(
				handle, ctx, 0, x, y, r,
				(d2_s32)(((d2_u32)((w + D2_FIX4(1)) / 2)) << (16 - 4)), 0, 1);

			control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_BAND1ENABLE |
				  (tmask & D2C_LIM1THRESHOLD);
			control = d2_initgradients_intern(handle, ctx, &bbox, 2, control);
			...
			/* inner circle */
			d2_circlesetup_intern(handle, ctx, 2, x, y, (d2_width)(r - (w / 2)),
					      (-D2_FIX16(1)) / 2, 1, 1);

			control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_LIM3ENABLE |
				  D2C_QUAD2ENABLE |
				  (tmask & (D2C_LIM1THRESHOLD | D2C_LIM3THRESHOLD));

			if (0 != (D2_DEV(handle)->hwrevision & D2FB_HILIMITERPRECISION)) {
				control |= (0 != (ctx->features & d2_feat_blur))
						   ? D2C_LIMITERPRECISION
						   : 0;
			}
...
d2_s32 d2_rendercircle_shadow(d2_device *handle, d2_point x, d2_point y, d2_width r, d2_width w)
{
	(void)d2_rendercircle_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline,
				     ...
				     (d2_point)(D2_DEV(handle)->soffy + y), r, w);
...
d2_s32 d2_rendercircle_outline(d2_device *handle, d2_point x, d2_point y, d2_width r, d2_width w)
{
	...
	if (w > 0) {
		...
		(void)d2_rendercircle_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x, y,
					     (d2_width)((r - (w / 2)) - (wo / 2)), wo);
	}
				    ...
				    d2_width w)
{
	...
	if ((w > 0) && (wo > 0)) {
		/* circle rings only */
		(void)d2_rendercircle_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x, y, r,
					     (d2_width)((wo * 2) + w));
	}
	if ((0 == w) && (wo > 0)) {
		/* solid circles only */

			    ...
			    d2_limdata *edge_buffer, d2_bbox *edge_bbox, const d2_s32 *connectors)
{
	...
	if (0 != (flags & d2_lei_miter_edge)) {
		if (d2_lj_bevel == ctx->linejoin) {
			...
			miter = (ctx->miterlimit + (w >> 1)) << (16 - 4);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
			wh = (d2_width)(((ctx->miterlimit + (w >> 1)) * 20) >> 4);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		}
	} else {
		miter = 0;
	...
	if ((ctx->features & (d2_feat_blur | d2_feat_aa)) == (d2_feat_blur | d2_feat_aa)) {
		...
		if (d2_lc_square == cap) {
			if (0 == (flags & d2_le_exclude_start)) {
				edge[1].start += (w16 - D2_FIX16(1)) >> 1;
					/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT
							     $*/
			}

			if (0 == (flags & d2_le_exclude_end)) {
				edge[2].start += (w16 - D2_FIX16(1)) >> 1;
					/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT
							     $*/
			}
		}
	...
	control = D2C_LIM1ENABLE | D2C_LIM2ENABLE | D2C_LIM5ENABLE | D2C_BAND1ENABLE |
		  (tmask & (D2C_LIM1THRESHOLD | D2C_LIM2THRESHOLD | D2C_LIM5THRESHOLD)) |
		  D2C_SPANABORT | D2C_SPANSTORE;
	...
	if (0 != (ctx->patmode & d2_pm_autoalign)) {
		...
		if (0 != (ctx->patmode & d2_pm_orthogonal)) {
			grad->xadd = (edge[1].yadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
			grad->yadd = (-edge[1].xadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		} else {
			grad->xadd = (edge[1].xadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
			grad->yadd = (edge[1].yadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}
		grad->xadd2 = ctx->patoffset << (16 - 4);
			/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/

		/* pattern auto advance */
		if (0 != (ctx->patmode & d2_pm_advance)) {
			ctx->patoffset += (len << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}
	}
	...
	if (d2_lc_round == cap) {
		...
		if (((x + (er * 2)) > 0) && ((y + (er * 2)) > 0) &&
			...
			/* include l3/l4 */
			control |= D2C_LIM3ENABLE | D2C_QUAD2ENABLE | D2C_UNIONAB |
				   (tmask & D2C_LIM3THRESHOLD);
	...
	if (0 != (flags & d2_le_exclude_start)) {
		...
		if (0 != (flags & d2_lei_ext_first_edge)) {
			/* use external edge */
			d2_triedge_setupnoaa_intern(&edge[1], x1, y1, connectors[0], connectors[1],
						    0);
			wh = (d2_width)(wh * 2);
				/* expand span delay (experimental) */ /* PRQA S 4131 */ /* $Misra:
											    #PERF_ARITHMETIC_SHIFT_LEFT
											    $*/
		} else {
			/* keep original edge */
			edge[1].start += D2_FIX16(1) >> 1;
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
			/* rightedge flags */
			if (dy > 0) {
	...
	if (0 != (flags & d2_le_exclude_end)) {
		...
		if (0 != (flags & d2_lei_ext_last_edge)) {
			/* use external edge */
			d2_triedge_setupnoaa_intern(&edge[2], (d2_point)(x2 - bbox.xmin),
						    (d2_point)(y2 - bbox.ymin), -connectors[2],
						    -connectors[3], 1);
			wh = (d2_width)(wh * 2);
				/* expand span delay (experimental) */ /* PRQA S 4131 */ /* $Misra:
											    #PERF_ARITHMETIC_SHIFT_LEFT
	...
	if (0 != (flags & d2_lei_miter_edge)) {
		/* miter outward shift to avoid line cutting into itself at 180 deg */
		miter += D2_FIX16(1);
			   ...
			   connectors[3]))) /* PRQA S 4130 */ /* $Misra: #PERF_BITWISE $ */
		{
			...
			d2_triedge_setupsqrt_intern(&edge[3], (d2_point)(x2 - bbox.xmin),
	...
	if (d2_lc_round == cap) {
		...
		/* new control word */
		control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_LIM3ENABLE |
			  (tmask & D2C_LIM1THRESHOLD) | D2C_LIM3THRESHOLD | D2C_SPANABORT;
		...
		if (0 != flip) {
			...
			dx = D2_INT4(bbox2.xmin - bbox.xmin);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
			dy = D2_INT4(bbox2.ymin - bbox.ymin);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		}

		/* D2_DLISTWRITES( D2_L3START, -edge[2].start - (dx * edge[2].xadd) - (dy *
		 * edge[2].yadd) + D2_FIX16(1) + D2_EPSILON ); */
		D2_DLISTWRITES(
			D2_L3START,
			(((-edge[2].start - (dx * edge[2].xadd)) - (dy * edge[2].yadd)) +
		...
		d2_startrender_intern(handle, &bbox2, (d2_u32)(w >> 4));
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	}
			     ...
			     d2_limdata *edge_buffer, d2_bbox *edge_bbox)
{
	...
	*/
	...
	/* calc edge interpolation parameters */
	edge[0].start = (((y1 * nx) - (x1 * ny)) >> 4) + D2_FIX16(1);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	edge[0].xadd = ny;
	edge[0].yadd = -nx;
	edge[1].start = (((x2 * ny) - (y2 * nx)) >> 4) + (D2_FIX16(1) >> 1);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	edge[1].xadd = -ny;
	edge[1].yadd = nx;
	...
	/* side parameters */
	if ((ctx->features & (d2_feat_blur | d2_feat_aa)) == (d2_feat_blur | d2_feat_aa)) {
		d2_triedge_setupblur_intern(&edge[2], (d2_point)(ax - bbox.xmin),
					    (d2_point)(ay - bbox.ymin), (ax - bx), (ay - by), ctx);
		d2_triedge_setupblur_intern(&edge[3], (d2_point)(dx - bbox.xmin),
					    (d2_point)(dy - bbox.ymin), (cx - dx), (cy - dy), ctx);
	} else {
		d2_triedge_setup_intern(&edge[2], (d2_point)(ax - bbox.xmin),
					(d2_point)(ay - bbox.ymin), (ax - bx), (ay - by), 0);
		d2_triedge_setup_intern(&edge[3], (d2_point)(dx - bbox.xmin),
					(d2_point)(dy - bbox.ymin), (cx - dx), (cy - dy), 0);
		edge[2].start += (D2_FIX16(1) >> 1);
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		edge[3].start += (D2_FIX16(1) >> 1);
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	}
	...
	if (d2_lc_square == cap) {
		if (0 == (flags & d2_le_exclude_start)) {
			edge[0].start += ((w1 << (16 - 4)) - D2_FIX16(1)) >> 1;
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}

		if (0 == (flags & d2_le_exclude_end)) {
			edge[1].start += ((w2 << (16 - 4)) - D2_FIX16(1)) >> 1;
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}
	}
	...
	if (0 != (ctx->patmode & d2_pm_autoalign)) {
		...
		if (0 != (ctx->patmode & d2_pm_orthogonal)) {
			grad->xadd = (edge[0].yadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
			grad->yadd = (-edge[0].xadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		} else {
			grad->xadd = (edge[0].xadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
			grad->yadd = (edge[0].yadd << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}
		...
		/* pattern auto advance */
		if (0 != (ctx->patmode & d2_pm_advance)) {
			ctx->patoffset += (l << 4) / ctx->patscale;
				/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
		}
	}
	...
	delay += (d2_u32)(((w1 > w2) ? w1 : w2) >> 4);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/

	control = D2C_LIM1ENABLE | D2C_LIM2ENABLE | D2C_LIM3ENABLE | D2C_LIM4ENABLE |
		  (tmask & (D2C_LIM1THRESHOLD | D2C_LIM2THRESHOLD | D2C_LIM3THRESHOLD |
			    D2C_LIM4THRESHOLD)) |
		  D2C_SPANABORT | D2C_SPANSTORE;
	...
	if (0 != d2_clipbbox_intern(handle, &bbox2)) {
		...
		d2_circlesetup_intern(handle, ctx, 0, (d2_point)xp, (d2_point)yp, (d2_width)r, 0, 0,
				      0);
		...
		if (0 != flip) {
			...
			nx = D2_INT4(bbox2.xmin - bbox.xmin);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
			ny = D2_INT4(bbox2.ymin - bbox.ymin);
				/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		}
		...
		D2_DLISTWRITES(D2_L3START,
			       edge[2].start + (edge[2].xadd * nx) + (edge[2].yadd * ny));
		D2_DLISTWRITES(D2_L3XADD, edge[2].xadd);
		D2_DLISTWRITES(D2_L3YADD, edge[2].yadd);
		D2_DLISTWRITES(D2_L4START,
			       edge[3].start + (edge[3].xadd * nx) + (edge[3].yadd * ny));
		D2_DLISTWRITES(D2_L4XADD, edge[3].xadd);
		D2_DLISTWRITES(D2_L4YADD, edge[3].yadd);
		D2_DLISTWRITES(D2_L5START, ((((D2_FIX16(1) - edge[l].start) - (edge[l].xadd * nx)) -
					     (edge[l].yadd * ny)) +
					    D2_EPSILON));
		D2_DLISTWRITES(D2_L5XADD, -edge[l].xadd);
		D2_DLISTWRITES(D2_L5YADD, -edge[l].yadd);

		control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_SPANABORT |
			  (ctx->thresholdmask &
			   (D2C_LIM1THRESHOLD | D2C_LIM3THRESHOLD | D2C_LIM4THRESHOLD)) |
			  D2C_LIM3ENABLE | D2C_LIM4ENABLE | D2C_LIM5ENABLE | D2C_LIM5THRESHOLD;
	...
	if (0 != cap2) {
		...
		(void)d2_renderlinedot_intern(handle, ctx, (d2_point)xp, (d2_point)yp, (d2_width)r,
					      1, edge_buffer, edge_bbox);
	}
			       ...
			       const d2_bbox *edge_bbox)
{
	...
	control = D2C_LIM1ENABLE | D2C_QUAD1ENABLE | D2C_SPANABORT | (D2C_SPANSTORE * 0) |
		  (tmask & D2C_LIM1THRESHOLD);

	control = d2_initgradients_intern(handle, ctx, &bbox, 2, control);

	delay = (d2_u32)(D2_CEIL4(er) >> 4);
		/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
	...
	if (0 != edges) {
		...
		xr = D2_INT4(edge_bbox->xmin - bbox.xmin);
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/
		yr = D2_INT4(edge_bbox->ymin - bbox.ymin);
			/* PRQA S 0502 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_RIGHT $*/

		if (0 != (edges & 1)) {
			D2_DLISTWRITES(D2_L3START, ((((D2_FIX16(1) - edge_buffer[0].start) +
		...
		if (0 != (edges & 2)) {
			D2_DLISTWRITES(D2_L4START, ((((D2_FIX16(1) - edge_buffer[1].start) +
...
d2_s32 d2_renderline_intern_split(d2_devicedata *handle, d2_contextdata *ctx, d2_point x1,
				  ...
				  d2_limdata *edge_buffer, d2_bbox *edge_bbox,
				  const d2_s32 *connectors)
{
	if (0 != (ctx->features & d2_feat_blur)) {
		d2_limdata tmp_edge_buffer[2];
		d2_bbox tmp_edge_bbox;

		(void)d2_renderline2_intern(handle, ctx, x1, y1, x2, y2, w, w, flags,
					    tmp_edge_buffer, &tmp_edge_bbox);
	} else {
		if ((w >= D2_FIX4(3)) && (d2_lc_round == ctx->linecap) &&
			...
			if (0 != d2_renderline_intern(handle, ctx, x1, y1, x2, y2, w,
						      flags | d2_le_exclude_both |
							      ...
							      d2_lei_buffer_last_edge,
						      edge_buffer, edge_bbox, connectors)) {
				if (0 == (flags & d2_le_exclude_start)) {
					(void)d2_renderlinedot_intern(handle, ctx, x1, y1, w, 1,
								      edge_buffer, edge_bbox);
				}

				if (0 == (flags & d2_le_exclude_end)) {
					(void)d2_renderlinedot_intern(handle, ctx, x2, y2, w, 2,
								      edge_buffer, edge_bbox);
				}
			}
		} else {
			(void)d2_renderline_intern(handle, ctx, x1, y1, x2, y2, w, flags,
						   edge_buffer, edge_bbox, connectors);
		}
	}
	return 1;
			   ...
			   d2_width w, d2_u32 flags)
{
	...
	(void)d2_renderline_intern_split(D2_DEV(handle), D2_DEV(handle)->ctxsolid, x1, y1, x2, y2,
					 w, flags, edge_buffer, &edge_bbox, NULL);
			       ...
			       d2_point y2, d2_width w, d2_u32 flags)
{
	...
	(void)d2_renderline_intern_split(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x1, y1, x2, y2,
					 w, flags, edge_buffer, &edge_bbox, NULL);
			     ...
			     d2_width w, d2_u32 flags)
{
	...
	(void)d2_renderline_intern_split(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x1, y1, x2, y2,
					 (d2_width)(w + (D2_DEV(handle)->outlinewidth * 2)), flags,
					 edge_buffer, &edge_bbox, NULL);
			    ...
			    d2_width w, d2_u32 flags)
{
	...
	(void)d2_renderline_intern_split(D2_DEV(handle), D2_DEV(handle)->ctxoutline,
					 (d2_point)(x1 + sx), (d2_point)(y1 + sy),
					 (d2_point)(x2 + sx), (d2_point)(y2 + sy), w, flags,
					 edge_buffer, &edge_bbox, NULL);
			    ...
			    d2_width w1, d2_width w2, d2_u32 flags)
{
	...
	(void)d2_renderline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxsolid, x1, y1, x2, y2, w1,
				    w2, flags, edge_buffer, &edge_bbox);
			      ...
			      d2_width w1, d2_width w2, d2_u32 flags)
{
	...
	(void)d2_renderline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, x1, y1, x2, y2,
				    (d2_width)(w1 + ow), (d2_width)(w2 + ow), flags, edge_buffer,
				    &edge_bbox);
			     ...
			     d2_width w1, d2_width w2, d2_u32 flags)
{
	...
	(void)d2_renderline2_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, (d2_point)(x1 + sx),
				    (d2_point)(y1 + sy), (d2_point)(x2 + sx), (d2_point)(y2 + sy),
				    w1, w2, flags, edge_buffer, &edge_bbox);

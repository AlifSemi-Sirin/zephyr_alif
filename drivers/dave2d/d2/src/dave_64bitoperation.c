...
#ifdef __CA850__
	tmp = (d2_u32)__satadd(a->low32, b->low32);
		/* PRQA S 3335 */ /* $Misra: #COMPILER_INTRINSIC $*/
#elif defined(__ICCV850__)
	tmp = __upper_mul64(a->low32, b->low32);
		/* PRQA S 3335 */ /* $Misra: #COMPILER_INTRINSIC $*/
#else
...
#ifdef __CA850__
	res->high32 = __mul32u(ui_a, ui_b_low32);
		/* PRQA S 3335 */ /* $Misra: #COMPILER_INTRINSIC $*/
#elif defined(__ICCV850__)
		...
		for (;;) {
			if (0 != (remainder & remMask)) {
				...
				if (0 == tmp64.high32) {
					...
					d2_div6432(&tmp64_2, divisor, &tmp64);
						/* PRQA S 3670 */ /* $Misra: #DIV64_RECURSION $*/
...
void d2_shiftleft64(const d2_int64 *var, d2_s32 index, d2_int64 *res)
{
	...
	res->high32 = (var->high32 << index) + (d2_s32)(var->low32 >> (32 - index));
...
{
	res->low32 = (d2_u32)(var->high32 << (32 - index)) + (var->low32 >> index);
		/* PRQA S 4131 */ /* $Misra: #PERF_ARITHMETIC_SHIFT_LEFT $*/
	res->high32 = var->high32 >> index;

				  ...
				  d2_u32 flags)
{
	...
	if (0 != (idxm & idym)) {
		if ((0 == ((dx1 | dy1) & ~7)) || /* PRQA S 4130 */ /* $Misra: #PERF_BITWISE $*/
	...
	switch (dxm & (idxm >> 1)) {
	case 0:
		bbox.xmin = (d2_point)D2_FLOOR4(x1);
		break;
	case 1:
		bbox.xmin = (d2_point)D2_FLOOR4(x2);
		break;
	case 2:
		bbox.xmin = (d2_point)D2_FLOOR4(x3);
		break;
	default:
		break;
	}
	...
	switch (idxm & (dxm >> 1)) {
	case 0:
		bbox.xmax = (d2_point)D2_CEIL4(x1);
		break;
	case 1:
		bbox.xmax = (d2_point)D2_CEIL4(x2);
		break;
	case 2:
		bbox.xmax = (d2_point)D2_CEIL4(x3);
		break;
	default:
		break;
	}
	...
	switch (dym & (idym >> 1)) {
	case 0:
		bbox.ymin = (d2_point)D2_FLOOR4(y1);
		break;
	case 1:
		bbox.ymin = (d2_point)D2_FLOOR4(y2);
		break;
	case 2:
		bbox.ymin = (d2_point)D2_FLOOR4(y3);
		break;
	default:
		break;
	}
	...
	switch (idym & (dym >> 1)) {
	case 0:
		bbox.ymax = (d2_point)D2_CEIL4(y1);
		break;
	case 1:
		bbox.ymax = (d2_point)D2_CEIL4(y2);
		break;
	case 2:
		bbox.ymax = (d2_point)D2_CEIL4(y3);
		break;
	default:
		break;
	}
...
{
	(void)d2_rendertri_intern(D2_DEV(handle), D2_DEV(handle)->ctxsolid, x1, y1, x2, y2, x3, y3,
				  flags);
			   ...
			   d2_point x3, d2_point y3, d2_u32 flags)
{
	...
	(void)d2_rendertri_intern(D2_DEV(handle), D2_DEV(handle)->ctxoutline, (d2_point)(x1 + sx),
				  (d2_point)(y1 + sy), (d2_point)(x2 + sx), (d2_point)(y2 + sy),
				  (d2_point)(x3 + sx), (d2_point)(y3 + sy), flags);
			    ...
			    d2_point x3, d2_point y3, d2_u32 flags)
{
	...
	case 0:
		...
		verts[4] = x3;
		verts[5] = y3;
		(void)d2_renderpolyline_intern(D2_DEV(handle), ctx, verts, NULL, 3, wo,
					       d2_le_closed, 0, 0);
		break;

...
#define D2_TEXORIGIN                                                                               \
	47 /*  address of first texture pixel (writing can trigger texturecache flush) */
...
#define D2C_READFORMAT3                                                                            \
	BIT(4) /* Textureformat bit2 ( 0 = alpha8, 1 = rgb565, 2 = argb8888, 3 = argb4444, 4 =     \
		  argb1555 ) */
#define D2C_READFORMAT4 BIT(5) /* Textureformat bit3 ( more formats )  */

#define D2C_WRITEFORMAT1 BIT(20) /* Framebuffer format bit0 */
#define D2C_WRITEFORMAT2                                                                           \
	BIT(21) /* Framebuffer format bit1 ( 00 = alpha8, 01 = rgb565, 10 = argb8888, 11=argb4444) \
		 */
#define D2C_WRITEFORMAT3 BIT(8) /* Framebuffer format bit2 ( 110 = rgba8888, 111=rgba4444) */

#define D2C_USE_ACB                                                                                \
	BIT(3) /* use alpha-blending instead of write-alpha for alpha channel (see D2C_WRITEALPHA) \
		*/
...
#define D2C_BSFA                                                                                   \
	BIT(6) /* Alpha channel blending: Blend Source Factor (alpha when set, 1 otherwise) */
#define D2C_BDFA                                                                                   \
	BIT(7) /* Alpha channel blending: Blend Destination Factor (alpha when set, 1 otherwise)   \
		*/
/*#define D2C_BC2A           BIT(3)*/ /* Alpha channel blending: Blend Color2 (Blend using Alpha of
					 Color2 as Dest when set) */
...
#define D2C_RLEFORMAT2                                                                             \
	BIT(31) /* RLE format bit1 00=1 byte per pixel; 01=2 byte per pixel; 10=3 byte per pixel;  \
		   11=4 byte per pixel */

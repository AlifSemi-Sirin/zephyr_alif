 ...
 *    d1_ma_separated   -  Video memory is separated from system memory and can be accessed by
 * transfer or mapping functions only. d1_ma_uncached    -  CPU access to mapped video memory is not
 * cached d1_ma_fulluma     -  Static arrays and malloced memory can be accessed by the hardware
 *    d1_ma_mapped      -  alloced video mem is static mapped into system mem -> use
 * <d1_mapfromvidmem> d1_ma_fullmapped  -  hardware can access malloced memory and static arrays  ->
 * use <d1_mapfromvidmem> and <d1_maptovidmem>
 *
 */
 ...
 *    constant   - true when layer allows constant color only (e.g. background) (use:
 * <d1_displaysetlayercolor>) alpha      - true when layer supports alpha (use:
 * <d1_displaysetlayeralpha>) window     - true when layer supports windowing (use:
 * <d1_displaysetlayerwindow>) swappable  - true when top-down position of layer can be changed
 * (use: <d1_displaysetlayerorder>) moveable     - true when the position of layer can be changed
 * (use <d1_displaysetlayerwindow>) resizeable   - true when layer can be resized if windowing is
 * enabled (use <d1_displaysetlayerwindow>) framecolour  - true when a framecolour can be set for
 * layer (use <d1_displaysetlayerframecolour>) alphalayer   - true when a alphalayer is available
 * for layer (use <d1_displaysetlayerenable>) pitch        - true when the width can be smaller than
 * the pitch (use <d1_displaysetlayerformat> and <d1_displaysetlayerwindow>) duplicate    - true
 * when the layer can be duplicated in horizontal and/or vertical direction (use
 * <d1_displaysetlayerduplication>) colourkey    - true when a colourkey can be set for the layer
 * (use <d1_displaysetlayercolourkey>) a1_modecount - number blendmodes for upper alpha supported by
 * layer a1_modes     - array of blendmodes for upper alpha supported by layer of type (<enum
 * d1_blendmodes>) (use <d1_displaysetlayerblendmode>) a0_modecount - number blendmodes for lower
 * alpha supported by layer a0_modes     - array of blendmodes for lower alpha supported by layer of
 * type (<enum d1_blendmodes>) (use <d1_displaysetlayerblendmode>)
 */
 ...
 *   videomodes    - array of output modes supported by display controller of type <struct
 * d1_videomode> layercount    - number of layers supported by display controller layercaps     -
 * array of layers capabilities of type <struct d1_layercaps> major_rev     - major hardware
 * revision of display controller minor_rev     - minor hardware revision of display controller
 ...
 *  Parameters:
 *    handle - device handle (see: <d1_opendevice>)
 *    ptr    - CPU accessible address pointing to a video memory block originally allocated using
 * <d1_allocvidmem>
 *
 *  Returns:
 ...
 *    To be sure, an asynchronous copy was finished, either start another copy or use see
 * <d1_cacheflush>.
 */
extern int d1_copytovidmem(d1_device *handle, void *dst, const void *src, unsigned int size,
			   int flags);
 ...
 */
extern int d1_copyfromvidmem(d1_device *handle, void *dst, const void *src, unsigned int size,
			     int flags);
 ...
 *
 *   Different hardware components and revisions offer different hardware registers (see: <Device
 * IDs>). Use this function to verify that a specific hardware interface is available on the current
 * host system.
 *
 *   Parameters:
 ...
 *     called (use <d1_getirqhandler>). Otherwise, e.g. in case of the vertical blank irq
 * (d1_irq_vbi), this will disable the display controller driver.
 *
 *   See also:
...
extern int d1_displaysetlayercolourkey(d1_device *handle, int layer, unsigned int colourkey,
				       int enable);

extern int d1_displaysetlayerduplication(d1_device *handle, int layer, int hor_duplication,
					 int ver_duplication);
 ...
 *  boolean success (false if layer is not capable)
 */
extern int d1_displaysetlayerwindow(d1_device *handle, int layer, int x, int y, int width,
				    int height);
 ...
 * parameters:
 *  handle   - device handle (see <d1_opendevice>)
 *  layer    - layer number starting at 0 (see <d1_displaygetcaps> for number of
 * layers/capabilities) position - new position of layer (0 is bottom)
 *
 ...
 *
 * Change pointer to frame buffer for non-constant layer immediately or synchronized to vertical
 * blanking.
 *
 * parameters:
 *  handle - device handle (see <d1_opendevice>)
...
#endif
extern di_fbo_t *d1_createdifbo(d1_device *handle, void *ptr, unsigned int width,
				unsigned int height, unsigned int pitch, int bpp);
